    ...
    <button @click="fetchData">Fetch Data</button>
      ...
      <table v-if="pollutionData">
        <thead>
          <tr>
            <th>State</th>
            <th>Total Water Pollutants In Pounds</th>
        ...
        <tbody>
          <tr v-for="(state, index) in pollutionData.state" :key="index">
            <td>{{ state }}</td>
            <td>{{ pollutionData.pollutantsvalue[index] }}</td>
  ...
  export default {
    ...
    methods: {
      async fetchData() {
        ...
        try {
          const response = await axios.get('http://localhost:8000/pollutants/' + newOp + '/' + newValue); // Update the URL with your server's actual address
