    ...
    <button @click="fetchData">Fetch Data</button>
      ...
      <table v-if="pollutionData">
        <thead>
          <tr>
            <th>State</th>
            <th>Person Days Exceeding PM2.5 Standard Value</th>
  ...
  export default {
    ...
    methods: {
      async fetchData() {
        ...
        try {
          const response = await axios.get('http://localhost:8000/pm25/' + newOp + '/' + newValue); // Update the URL with your server's actual address
